...
def artPath():
    ...
    if theme in ['-', '']: return
        ...
        return os.path.join(xbmcaddon.Addon('script.scorpion.artwork').getAddonInfo('path'), 'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1').lower() if condVisibility('System.HasAddon(script.scorpion.artwork)') else setting('appearance.alt').lower()
    return appearance
...
def metaFile():
    if condVisibility('System.HasAddon(script.scorpion.metadata)'):
        return os.path.join(xbmcaddon.Addon('script.scorpion.metadata').getAddonInfo('path'), 'resources', 'data', 'meta.db')
